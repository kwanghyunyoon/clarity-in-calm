  ...
  const renderEntry = useCallback(({ item: entry, index: i }: ListRenderItemInfo<EntryItem>) => {
    ...
    if (isSealed) {
      ...
      return (
        ...
        >
          <Ionicons name="mail-outline" size={48} color={colors.primary} style={styles.sealedEnvelope} />
      ...
      >
        {entry.isFutureSelf && (
          <View style={[styles.futureSelfBadge, { backgroundColor: colors.primary + '18' }]}>
            <View style={styles.futureSelfBadgeRow}>
          ...
          <View style={styles.entryMoodRow}>
            ...
            {entry.templateId && (
              <View style={[styles.templateBadge, { backgroundColor: colors.backgroundElement }]}>
                <Ionicons
                  name={JOURNAL_TEMPLATES.find(t => t.id === entry.templateId)?.icon ?? 'document-text-outline'}
                  size={11}
...
const styles = StyleSheet.create({
  ...
  futureSelfBadgeRow:  { flexDirection: 'row', alignItems: 'center', gap: 4 },
